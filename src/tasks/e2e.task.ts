    ...
    return (skipWebDriverUpdate ? Promise.resolve(0) : webDriverUpdate()).then(exitCode => {
        if (exitCode === 0) {
            ...
            if (specs) {
                specPaths = specs
                    .split(",")
                    .map(spec => path.resolve(path.join(__dirname, "..", "config"), path.join(config.rootDir, spec)))
                    .join(",");
            }
...
function runProtractor(port: number, specs: string) {
    return new Promise(resolve => {
        ...
        const protractorProcess = spawn(
            getProtractorBinaryPath("protractor"),
            [configPath, "--specs", specs, "--baseUrl", `http://localhost:${port}`],
            { stdio: "inherit" }
        );
